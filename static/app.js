...
document.getElementById("predict-button").addEventListener("click", async () => {
    ...
    // Prepare payload
    const payload = {
        pickup_datetime: new Date().toISOString(), // Current date/time
    ...
    try {
        ...
        if (result.error) {
            ...
            const resultText = Object.entries(result)
                .map(([model, prediction]) => `${model}: ${prediction.toFixed(2)} seconds`)
                .join("\n");
...
document.getElementById("fetch-results").addEventListener("click", async () => {
    try {
        ...
        if (results.error) {
            document.getElementById("evaluation-results").innerText = `Error: ${results.error}`;
        } else {
            const resultsText = Object.entries(results)
                .map(([model, metrics]) => `${model} - Mean Squared Error: ${metrics.mean_squared_error.toFixed(2)} seconds²`)
                .join("\n");
            document.getElementById("evaluation-results").innerText = resultsText;
        }
    } catch (error) {
        document.getElementById("evaluation-results").innerText = `Error: ${error.message}`;
    }
});
